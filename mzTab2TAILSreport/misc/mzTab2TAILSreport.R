...
calculateIntensityFC <- function(peptide.data) {
  ...
  peptide.data[which(peptide.data$fc.H.M < -FcCutoff),]$fc.H.M <- -FcCutoff
  peptide.data[which(peptide.data$fc.M.L < -FcCutoff),]$fc.M.L <- -FcCutoff
...
# add column with amino acid at position P1
...
idx.HM.mInf <- which(peptide.data$fc.H.M == -FcCutoff)
idx.HM.zero <- which((peptide.data$fc.H.M > -FcCutoff) & (peptide.data$fc.H.M < FcCutoff))
idx.HM.pInf <- which(peptide.data$fc.H.M == FcCutoff)

idx.ML.mInf <- which(peptide.data$fc.M.L == -FcCutoff)
idx.ML.zero <- which((peptide.data$fc.M.L > -FcCutoff) & (peptide.data$fc.M.L < FcCutoff))
idx.ML.pInf <- which(peptide.data$fc.M.L == FcCutoff)
